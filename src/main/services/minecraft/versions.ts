...
/** Writes a loader-generated manifest into the shared versions folder. */
...
/** Loads a version manifest, downloading it from Mojang when it is a vanilla id. */
...
export interface ResolvedVersion extends VersionJson {
  /** Full inheritance chain, closest ancestor last. */
  chain: string[]
  javaMajor: number
}

/** Follows `inheritsFrom` until a root manifest, merging as it goes. */
...
function libraryDownloadUrl(library: Library, relativePath: string): string | null {
  ...
  // Sane defaults for loader manifests that give neither `downloads` nor `url`.
...
export function resolveLibraries(version: VersionJson): ResolvedLibrary[] {
  ...
  for (const library of version.libraries ?? []) {
    ...
    // Plain artifact (skip when the library is natives-only).
...
/** Classpath entries, newest declaration of each group:artifact winning. */
...
async function installAssets(version: VersionJson, options: InstallOptions): Promise<void> {
  ...
  // Pre-1.7 versions read assets from a flat "virtual" tree instead of the hashed store.
...
/** Extracts every native jar for this version into a per-version natives folder. */
  ...
  for (const native of natives) {
    ...
    await extractZip(native.path, targetDir, {
      filter: (name) => {
        ...
        // Only platform binaries and their sidecars are useful at runtime.
        return true
      }
    })
  }

  // Some packages nest binaries a level deep; hoist them so java.library.path works.
...
/**
 * Ensures the client jar, every library, natives, assets and the log4j config
 * for a resolved version are on disk.
 */
export async function installVersion(versionId: string, options: InstallOptions = {}): Promise<ResolvedVersion> {
  ...
  // 1. Client jar — download from the root of the inheritance chain.
  ...
  // 4. Log4j2 configuration — gives us structured, parseable game logs.
...
/** Assets root the game should read from, honouring legacy layouts. */
